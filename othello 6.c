...
#include <string.h>
#include <ctype.h>
#include "othello.h" // created a header file to hold all preprocessor commands
...
void playGame()
{
    ...
    while(endGame == FALSE) // updated to loop until the end of the game
    {
        endGame = isEndGame(board);

        if(endGame == TRUE)
        {
            gameOver(board, playerX, playerO); // displays game over screen once hit end of game
            break; // stops looping the game
        }
...
int checkDiagonal(int row, int col, char board[ROW][COL], char playerChar) // checks how many discs of opposing player would be flipped
{
    int count = 0;
    int r = row;
    int c = col;
...
void updateDiagonal(int row, int col, char board[ROW][COL], struct Player *player) // flips discs of opposing player
{
    int r = row;
    int c = col;
...
int isEndGame(char board[ROW][COL]) // tests end game situations
{
    // tests the end game situation of a full board
//    setFullBoard(board);
//    displayBoard(board);

    if(getDiscCount(board) == SQUARES) // checks if game board is full
    {
        return TRUE;
    }

    // tests the end game situation of no valid moves left
//    setNoMoveBoard(board);
//    displayBoard(board);

    if(validMoveExists(board,'X') == FALSE && validMoveExists(board,'O') == FALSE) // checks if no valid moves left by either player
    {
        return TRUE;
    }
    return FALSE;
...
int getDiscCount(char board[ROW][COL]) // function counts discs on board
{
    int disc = ZERO;

    // for loop iterates through the board
    for(int row = 0; row <= MAX; row++)
        for(int col = 0; col <= MAX; col++)
...
int validMoveExists(char board[ROW][COL], char playerChar) // function determines if valid move exists for either player
{
    int spaces = ZERO;
    int result = FALSE;

    // for loops iterates through the board
    for(int row = 0; row <= MAX; row++)
        for(int col = 0; col <= MAX; col++)
        {
            if(board[row][col] == SPACE)
            ...
            {
                if(checkHorizontal == ZERO && checkVertical == ZERO && checkDiagonal == ZERO)
                {
                    result = FALSE; // there is no valid move
                }
                else
                {
                    result = TRUE;
...
void gameOver(char board[ROW][COL], struct Player playerX, struct Player playerO) // function for end of game
{
    // displays end game banner
    printf("*******************************************************\n");
    printf("********************** GAME OVER **********************\n");
    printf("*******************************************************\n");
